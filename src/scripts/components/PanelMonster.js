...
class PanelMonster extends Component {
  ...
  attachListeners() {
      ...
      function () {
        $("#modal-delete").one(
          ...
          function () {
            ...
            Router.setUrl("/vault", {
              showModal: "modal-deleted-monster",
              oldId: this.data.id,
            });
          }.bind(this)
        );
    ...
    $(this.el).on(
      "click",
      "#modal-new-monster .btn-add-another",
      function () {
        $("#modal-new-monster").one(
          "hidden.bs.modal",
          function () {
  ...
  exportMonster() {
    ...
    let name =
      monster.blueprint.getName() == null
        ? "Monster"
        : monster.blueprint.getName();
  ...
  /**
   * Exports the monster to the clipboard in LgendKeeper format.
   */
  showLegendKeeper() {
    let blueprint = this.children["blueprint"].data.blueprint;
