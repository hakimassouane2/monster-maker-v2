...
const Frankenstein = (function () {
  ...
  function createMonster(blueprint) {
    ...
    if (
      blueprint.getRank() == "solo" &&
      blueprint.getPhase() != null &&
      blueprint.getPhases() > 1
    ) {
    ...
    monster.setImageType(blueprint.getImageType());
    ...
    if (blueprint.getMethod() == "quickstart") {
      monster.setAcValue(
        quickstarter.getAc() +
          (blueprint.getAcModifier() == null ? 0 : blueprint.getAcModifier())
      );
    } else {
      monster.setAcValue(
    ...
    if (blueprint.getMethod() == "quickstart") {
      ...
      monster.setHpAverage(
        Math.floor(
          quickstarter.getHp() * phaseHp +
            (blueprint.getHpModifier() == null ? 0 : blueprint.getHpModifier())
        )
      );
    } else {
      monster.setHpAverage(
    ...
    // Set speeds
    let speeds = [];
    ["Normal", "Burrow", "Climb", "Fly", "Swim", "Other"].forEach(function (
      field
    ) {
    ...
    ["Str", "Dex", "Con", "Int", "Wis", "Cha"].forEach(function (field) {
      let ability = null;
      if (blueprint.getMethod() == "quickstart") {
        let modifier = quickstarter.getAbility(
        ...
        let score =
          blueprint["getAbility" + field]() == null
            ? 0
    ...
    if (blueprint.getMethod() == "quickstart") {
      ...
      blueprint.getSavingThrowsQuickstart().forEach(function (savingThrow) {
        ...
        if (blueprint.getSavingThrowsQuickstartMode() == "sync") {
          modifier = quickstarter.getSavingThrow(
            blueprint.getAbilityQuickstartRank(savingThrow.ability)
          );
        } else {
          modifier = quickstarter.getSavingThrow(
            blueprint.getSavingThrowQuickstartRank(savingThrow.ability)
          );
        }
      ...
      blueprint.getSavingThrowsManual().forEach(function (savingThrow) {
        if (
          savingThrows.findIndex((x) => x.ability == savingThrow.ability) == -1
        ) {
          savingThrows.push({
            ability: savingThrow.ability,
            modifier:
              getMonsterAbilityModifier(monster, savingThrow.ability) +
    ...
    blueprint.getSkills().forEach(function (skill) {
      ...
      if (
        name != null &&
        skill != null &&
        skills.findIndex((x) => x.name == name) == -1
      ) {
        if (skill.name == "custom") {
          skills.push({
            name: skill.custom.name,
            modifier:
              getMonsterAbilityModifier(monster, skill.custom.ability) +
              getMonsterProficiencyBonus(monster, skill.proficiency),
          });
        } else {
          ...
          skills.push({
            name: skill.name,
            modifier:
              getMonsterAbilityModifier(monster, targetAbility) +
              getMonsterProficiencyBonus(monster, skill.proficiency),
          });
        }
      }
    ...
    // Set senses
    let senses = [];
    ["Blindsight", "Darkvision", "Tremorsense", "Truesight", "Other"].forEach(
      function (field) {
    ...
    if (blueprint.getMethod() == "quickstart") {
      ...
      monster.setAttackDamage(
        quickstarter.getDamage() +
          (blueprint.getDamageModifier() == null
            ? 0
            : blueprint.getDamageModifier())
      );
    ...
    blueprint.getResistances().forEach(function (resistance) {
      let type =
        resistance.type == "custom" ? resistance.custom : resistance.type;
    ...
    // Set free actions
    let freeActions = [];
    blueprint.getFreeActions().forEach(function (freeAction) {
      if (freeAction.name != null || freeAction.detail != null) {
        freeActions.push({
          name: freeAction.name,
          rarity: freeAction.rarity,
          type: freeAction.type,
          detail: parseMarkdown(freeAction.detail, monster),
        });
      }
    });
    monster.setFreeActions(freeActions);
    ...
    if (blueprint.getParagonActionsType() == "custom") {
      monster.setParagonActions(
        blueprint.getParagonActionsAmount() == null
          ? 0
          : blueprint.getParagonActionsAmount()
      );
    ...
    monster.setLegendaryActionsPerRound(
      blueprint.getLegendaryActionsPerRound() == null
        ? 0
        : blueprint.getLegendaryActionsPerRound()
    );
    ...
    blueprint.getNotes().forEach(function (note) {
      if (
        note != null &&
        note.detail != null &&
        notes.findIndex((x) => x == note.detail) == -1
      ) {
  ...
  function getMonsterAbilityModifier(monster, ability) {
    let field =
      "getAbility" + ability.charAt(0).toUpperCase() + ability.slice(1);
    ...
    try {
      ...
      if (monster.getLevel() != "—") {
        ...
        if (monster.getAttackIsActive()) {
          output = output.replace(
            /(.*?)(\battack\b)(.*?)/g,
            "DC $1" + monster.getAttackBonus() + "$3"
          );
        } else {
          output = output.replace(
            /(.*?)(\battack\b)(.*?)/g,
            "+$1" + monster.getAttackBonus() + "$3"
          );
        }

        output = output.replace(/\bdamage\b/g, monster.getAttackDamage());
        output = output.replace(
          /\bdc-primary-bonus\b/g,
          monster.getDcPrimary()
        );
        output = output.replace(
          /(.*?)(\bdc-primary\b)(.*?)/g,
      ...
      output = output.replace(
        /(.*?)(\bstr-save\b)(.*?)/g,
        "+$1" + monster.getSavingThrow("str").modifier + "$3"
      );
      ...
      output = output.replace(
        /(.*?)(\bdex-save\b)(.*?)/g,
        "+$1" + monster.getSavingThrow("dex").modifier + "$3"
      );
      ...
      output = output.replace(
        /(.*?)(\bcon-save\b)(.*?)/g,
        "+$1" + monster.getSavingThrow("con").modifier + "$3"
      );
      ...
      output = output.replace(
        /(.*?)(\bint-save\b)(.*?)/g,
        "+$1" + monster.getSavingThrow("int").modifier + "$3"
      );
      ...
      output = output.replace(
        /(.*?)(\bwis-save\b)(.*?)/g,
        "+$1" + monster.getSavingThrow("wis").modifier + "$3"
      );
      ...
      output = output.replace(
        /(.*?)(\bcha-save\b)(.*?)/g,
        "+$1" + monster.getSavingThrow("cha").modifier + "$3"
      );
      ...
      output = output.replace(
        /\bac\b/g,
        monster.getAcValue() == "—" ? 0 : monster.getAcValue()
      );
      output = output.replace(
        /\bhp\b/g,
        monster.getHpAverage() == "—" ? 0 : monster.getHpAverage()
      );

      output = output.replace(/\[(.*?)(, *?d(\d+))?\]/g, (token, t1, t2, t3) =>
        numberToRandom(token, t1, t3)
      );
