...
const Exporters = (function () {
  function blueprintToImprovedInitiative(blueprint) {
    ...
    return {
      ...
      Senses: monster
        .getSenses()
        .filter((x) => x.type != "passive Perception")
        .map(function (x) {
          return x.type + " " + x.value;
        }),
    ...
    console.log({
      ...
      data: {
        ...
        attributes: {
          ...
          movement: {
            burrow: getOptionalValue(
              monster.getSpeeds().find((x) => x.type == "burrow")
            ),
            climb: getOptionalValue(
              monster.getSpeeds().find((x) => x.type == "climb")
            ),
            fly: getOptionalValue(
              monster.getSpeeds().find((x) => x.type == "fly")
            ),
            swim: getOptionalValue(
              monster.getSpeeds().find((x) => x.type == "swim")
            ),
            walk: getOptionalValue(
              monster.getSpeeds().find((x) => x.type == "normal")
            ),
            units: "ft",
            hover: false,
          },
          senses: {
            darkvision: getOptionalValue(
              monster.getSenses().find((x) => x.type == "darkvision")
            ),
            blindsight: getOptionalValue(
              monster.getSenses().find((x) => x.type == "blindsight")
            ),
            tremorsense: getOptionalValue(
              monster.getSenses().find((x) => x.type == "tremorsense")
            ),
            truesight: getOptionalValue(
              monster.getSenses().find((x) => x.type == "truesight")
            ),
            units: "ft",
            special: "",
          },
  ...
  function blueprintToLegendKeeper(blueprint) {
    let monster = Frankenstein.createMonster(blueprint);
    return Handlebars.templates["MonsterLegendKeeper"](monster);
  }
  ...
  return {
    ...
    blueprintToLegendKeeper: blueprintToLegendKeeper,
  };
})();
