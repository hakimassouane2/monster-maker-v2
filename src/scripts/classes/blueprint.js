...
class Blueprint {
  ...
  load_v0(options) {
    ...
    this.description.name = options.name
      ? parseString(options.name)
      : this.description.name;
    this.description.level = options.level
      ? parseNumber(options.level)
      : this.description.level;
    this.description.role = options.role
      ? parseString(options.role.toLowerCase())
      : this.description.role;
    this.description.rank = options.type
      ? parseString(options.type.toLowerCase())
      : this.description.rank;
    this.description.players = options.partySize
      ? parseNumber(options.partySize)
      : this.description.players;
    let abilitiesQuickstart =
      options.abilities && Array.isArray(options.abilities)
        ? options.abilities.map(function (ability) {
  ...
  load_v1(options) {
    ...
    this.description = {
      name: options.description
        ? parseString(options.description.name)
        : this.description.name,
      size: options.description
        ? parseString(options.description.size)
        : this.description.size,
      type: options.description
        ? parseString(options.description.type)
        : this.description.type,
      alignment: options.description
        ? parseString(options.description.alignment)
        : this.description.alignment,
      level: options.description
        ? parseNumber(options.description.level)
        : this.description.level,
      role: options.description
        ? parseString(options.description.role)
        : this.description.striker,
      rank: options.description
        ? parseString(options.description.rank)
        : this.description.standard,
      players: options.description
        ? parseNumber(options.description.players)
        : this.description.players,
      phase: options.description
        ? parseNumber(options.description.phase)
        : this.description.phase,
      phases: options.description
        ? parseNumber(options.description.phases)
        : this.description.phases,
    };
    ...
    this.display = {
      theme:
        options.display && options.display.theme
          ? parseString(options.display.theme)
          : this.display.theme,
      columns:
        options.display && options.display.columns
          ? parseNumber(options.display.columns)
          : this.display.columns,
      image: {
        url:
          options.display && options.display.image
            ? parseString(options.display.image.url)
            : this.display.image.url,
        type:
          options.display && options.display.image
            ? parseString(options.display.image.type)
            : this.display.image.type,
        position:
          options.display && options.display.image
            ? parseString(options.display.image.position)
            : this.display.image.position,
      },
    };
    this.ac = {
      base: options.ac ? parseNumber(options.ac.base) : this.ac.base,
      modifier: options.ac
        ? parseNumber(options.ac.modifier)
        : this.ac.modifier,
    ...
    this.hp = {
      ...
      modifier: options.hp
        ? parseNumber(options.hp.modifier)
        : this.hp.modifier,
      roll: options.hp ? parseString(options.hp.roll) : this.hp.roll,
    };
    this.damage = {
      modifier: options.damage
        ? parseNumber(options.damage.modifier)
        : this.damage.modifier,
    };
    this.speed = {
      normal: options.speed
        ? parseString(options.speed.normal)
        : this.speed.normal,
      burrow: options.speed
        ? parseString(options.speed.burrow)
        : this.speed.burrow,
      climb: options.speed
        ? parseString(options.speed.climb)
        : this.speed.climb,
      ...
      other: options.speed
        ? parseString(options.speed.other)
        : this.speed.other,
    };
    let abilitiesQuickstart = options.abilities
      ? options.abilities.quickstart
      : this.abilities.quickstart;
    this.abilities = {
      str: options.abilities
        ? parseNumber(options.abilities.str)
        : this.abilities.str,
      dex: options.abilities
        ? parseNumber(options.abilities.dex)
        : this.abilities.dex,
      con: options.abilities
        ? parseNumber(options.abilities.con)
        : this.abilities.con,
      int: options.abilities
        ? parseNumber(options.abilities.int)
        : this.abilities.int,
      wis: options.abilities
        ? parseNumber(options.abilities.wis)
        : this.abilities.wis,
      cha: options.abilities
        ? parseNumber(options.abilities.cha)
        : this.abilities.cha,
    ...
    let savingThrowsManual = parseArray(
      options.savingThrows ? options.savingThrows.manual : [],
    ...
    let savingThrowsQuickstartMode = options.savingThrows
      ? options.savingThrows.quickstartMode
      : this.savingThrows.quickstartMode;
    let savingThrowsQuickstart = options.savingThrows
      ? options.savingThrows.quickstart
      : this.savingThrows.quickstart;
    this.savingThrows = {
    ...
    this.vulnerabilities = parseArray(
    ...
    this.conditions = parseArray(options.conditions, function (condition) {
      return {
        type: parseString(
          condition.type == "unconcious" ? "unconscious" : condition.type
        ),
        custom: parseString(condition.custom),
      };
    });
    this.senses = {
      blindsight: options.senses
        ? parseString(options.senses.blindsight)
        : this.senses.blindsight,
      darkvision: options.senses
        ? parseString(options.senses.darkvision)
        : this.senses.darkvision,
      tremorsense: options.senses
        ? parseString(options.senses.tremorsense)
        : this.senses.tremorsense,
      truesight: options.senses
        ? parseString(options.senses.truesight)
        : this.senses.truesight,
      other: options.senses
        ? parseString(options.senses.other)
        : this.senses.other,
    };
    ...
    this.paragonActions = {
      type: options.paragonActions
        ? parseString(options.paragonActions.type)
        : this.paragonActions.type,
      amount: options.paragonActions
        ? parseNumber(options.paragonActions.amount)
        : this.paragonActions.amount,
    };
    this.legendaryActionsPerRound = parseNumber(
      options.legendaryActionsPerRound
    );
    this.legendaryActions = parseArray(
  ...
  getImageType() {
    return this.display.image.type;
  }
